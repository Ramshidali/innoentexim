...
def index(request):
    ...
    monthly_purchase = PurchasedItems.objects.filter(purchase__date__month=datetime.today().date().month,is_deleted=False).aggregate(total_amount=Sum('amount'))['total_amount'] or 0
    monthly_purchase_expense = PurchaseExpense.objects.filter(purchase__date__month=datetime.today().date().month,is_deleted=False).aggregate(total_amount=Sum('amount'))['total_amount'] or 0
    monthly_sales = SalesItems.objects.filter(sales__date__month=datetime.today().date().month,is_deleted=False).aggregate(total_amount=Sum('amount_in_inr'))['total_amount'] or 0
    monthly_sales_expense = SalesExpenses.objects.filter(sales__date__month=datetime.today().date().month,is_deleted=False).aggregate(total_amount=Sum('amount_in_inr'))['total_amount'] or 0
    monthly_other_expences = OtherExpences.objects.filter(date_added__month=datetime.today().date().month,is_deleted=False).aggregate(total_amount=Sum('amount'))['total_amount'] or 0
    
    monthly_expenses = monthly_purchase + monthly_sales_expense + monthly_other_expences
    monthly_profit = monthly_sales - monthly_purchase - monthly_purchase_expense - monthly_sales_expense - monthly_other_expences
